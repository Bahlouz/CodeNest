...
const detectLanguageRoute = require("./routes/detectLanguageRoute");
...
app.use("/api/detect_language", detectLanguageRoute);

// New routes for executing code
...
app.post("/api/run-python", (req, res) => {
  ...
  fs.writeFile(filePath, code, (fileErr) => {
    if (fileErr) {
      return res.status(500).json({ error: "Failed to write Python code to file" });
    }
...
app.post("/api/run-java", (req, res) => {
  ...
  fs.writeFile(filePath, code, (fileErr) => {
    if (fileErr) {
      return res.status(500).json({ error: "Failed to write Java code to file" });
    }
...
app.post("/api/run-php", (req, res) => {
  ...
  fs.writeFile(filePath, code, (fileErr) => {
    if (fileErr) {
      return res.status(500).json({ error: "Failed to write PHP code to file" });
    }
...
app.post("/api/run-typescript", (req, res) => {
  ...
  fs.writeFile(filePath, code, (fileErr) => {
    if (fileErr) {
      return res.status(500).json({ error: "Failed to write TypeScript code to file" });
    }

    
    exec(`tsc ${filePath} --outFile ${jsFilePath}`, (compileErr, stdout, stderr) => {
      if (compileErr) {
        return res.status(500).json({ error: stderr || compileErr.message } );
      }
...
app.post("/api/run-ruby", (req, res) => {
  ...
  fs.writeFile(filePath, code, (fileErr) => {
    if (fileErr) {
      return res.status(500).json({ error: "Failed to write Ruby code to file" });
    }
...
app.post("/api/run-go", (req, res) => {
  ...
  fs.writeFile(filePath, code, (fileErr) => {
    if (fileErr) {
      cleanUp(tempDir);
      return res.status(500).json({ error: "Failed to write Go code to file." });
    }
